...
15. [US Federal and Payment Card Compliance Overlays](#15-us-federal-and-payment-card-compliance-overlays)
16. [Quarterly AI Transparency Note](#16-quarterly-ai-transparency-note)
...
#### Applicable STIGs by Infrastructure Layer

| Infrastructure Layer | Applicable STIG(s) | AI Governance Integration Point |
|---------------------|--------------------|---------------------------------|
| **Host OS (RHEL/Linux)** | RHEL 8/9 STIG, General Purpose OS SRG | Inference endpoint hardening; journald log forwarding to SIEM; file permission controls on model artifact directories (ties to CM-6 baseline). |
| **Container Runtime** | Docker Enterprise STIG, Kubernetes STIG | Immutable container images for model serving; no privileged containers for LLM agents; pod security policies enforce least agency. |
| **Web/API Services** | Web Server SRG, Application Server SRG | AI gateway and inference API endpoints must meet SRG requirements: TLS 1.2+ (FIPS-compliant), session management, and error handling. |
| **Network Infrastructure** | Network Infrastructure Policy STIG | VLAN segmentation for inference endpoints; ACLs restricting LLM egress to approved gateway only; IDS/IPS coverage on AI traffic. |
| **Database / Vector Store** | PostgreSQL STIG, Oracle STIG (as applicable) | Vector database hosting embeddings must meet STIG database requirements: authentication, audit logging, encryption at rest, privilege separation. |
| **PKI / Cryptography** | PKI and Certificate STIG | mTLS certificates must use DoD-approved PKI or equivalent; FIPS 140-3 validated cryptographic modules required. |

#### STIG Compliance Integration with AI Governance Artifacts

- **Model Card additions:** Each Model Card must document the STIG compliance status of the hosting infrastructure, including STIG version, benchmark date, and any open findings with associated POA&Ms.
- **CM-6 baseline extension:** The AI Configuration Management baseline must reference the applicable STIG benchmark as the authoritative hardening standard for each infrastructure layer. STIG findings are treated as configuration drift and tracked in the same change management workflow.
...
#### Key ARS Control Areas and AI-Specific Implications

| ARS Control Family | Key Controls | AI Governance Implication |
|-------------------|-------------|---------------------------|
| **Access Control (AC)** | AC-2, AC-3, AC-6, AC-17 | RBAC role assignments must satisfy ARS AC-2 account management requirements. Remote access to inference endpoints (AC-17) requires MFA and encrypted sessions. Least privilege (AC-6) maps directly to the Least Agency principle for LLM agents. |
| **Audit and Accountability (AU)** | AU-2, AU-3, AU-6, AU-9, AU-12 | ARS tightens AU-2 event selection and AU-3 content requirements beyond baseline 800-53. AU-9 (protection of audit information) requires tamper-evident log storage — critical for AI forensic integrity. |
| **Configuration Management (CM)** | CM-2, CM-6, CM-7, CM-8 | ARS CM-6 ODPs must be explicitly documented for AI configuration items. CM-7 (least functionality) directly supports the Anti-Agency Guardrail — disable all AI agent capabilities not explicitly required. |
| **Identification & Authentication (IA)** | IA-2, IA-5, IA-8 | MFA required for all administrative access to AI infrastructure. IA-5 authenticator management applies to API keys and service tokens — short-lived tokens are required, not optional. |
| **Risk Assessment (RA)** | RA-3, RA-5, RA-7 | ARS RA-3 requires a formal risk assessment for AI use cases — the AIIA template satisfies this requirement when completed with ARS-required content fields. RA-5 vulnerability scanning must include AI-specific libraries. |
| **System & Comm. Protection (SC)** | SC-7, SC-8, SC-28 | AI gateway boundary protection (SC-7) must be documented in system architecture diagrams. SC-8 requires encryption for all prompt/output data in transit. SC-28 requires encryption at rest for model artifacts and vector store contents. |
| **Supply Chain Risk Mgmt (SR)** | SR-3, SR-5, SR-6 | Third-party model intake must satisfy ARS SR controls: provenance verification (SR-3), supplier assessments (SR-5), and tamper resistance/detection (SR-6). The Vendor Intake Questionnaire maps directly to these requirements. |
| **Privacy (PT / AP)** | AP-1, AP-2, DI-1, DM-1 | For AI systems processing PHI, ARS privacy controls require documented data inventories, purpose specifications, and data minimization plans. |

#### ARS-Specific Requirements Not Covered by Base 800-53

- **CMS Privacy Act compliance:** AI systems that make or inform decisions about CMS beneficiaries must include Privacy Act notices and maintain Systems of Records Notices (SORNs) where applicable.
- **Business Associate Agreements (BAA):** Any third-party AI vendor or model provider that processes PHI on behalf of a CMS-connected entity requires a signed BAA. The Vendor Intake Questionnaire must include BAA status as a required field.
...
#### Cardholder Data Environment (CDE) Scoping for AI Systems

Incorrect scoping is the most common PCI DSS failure mode. Scope determination is required in the AIIA for any financially-adjacent use case.

- **In-scope triggers:** An AI system is in scope if it processes CHD/SAD directly; if it has network connectivity to in-scope systems; if it stores prompt history or outputs containing CHD; or if a compromise of the AI system could affect the security of CHD.
- **RAG and vector store scoping:** If the vector database contains documents with CHD (e.g., transaction records used as RAG context), the vector store and its ingestion pipeline are in scope. **Tokenization of CHD before ingestion into RAG pipelines is strongly recommended** to limit scope expansion.
- **Gateway scoping:** The AI gateway is in scope if it proxies requests containing CHD. Gateway redaction rules must be configured to strip CHD before forwarding to external model APIs.

#### PCI DSS v4.0 Requirements Mapped to AI Governance Controls

| PCI DSS Req. | Requirement Summary | AI Governance Mapping |
|-------------|--------------------|-----------------------|
| **Req. 1 & 2** | Network security controls; secure configurations | AI inference endpoints and gateways must be in documented network diagrams. STIG-aligned hardening satisfies Req. 2. No AI component may use vendor-supplied default credentials. |
| **Req. 3** | Protect stored account data | CHD must not be stored in prompt logs, model outputs, or vector stores without strong cryptography (AES-256). Tokenization of PANs before any AI processing is the preferred control. |
| **Req. 4** | Protect CHD in transit | mTLS required for all AI data flows involving CHD. TLS 1.2 minimum; TLS 1.3 strongly preferred. Certificate management falls under the Gateway_Admin role. |
| **Req. 6** | Secure systems and software | AI gateways and inference APIs are web-facing applications subject to Req. 6. PCI DSS v4.0 Req. 6.4 requires protection against web application attacks — **prompt injection is a directly relevant attack vector.** |
| **Req. 7** | Restrict access by business need | The RBAC model in Section 5 satisfies Req. 7. Role assignments must be reviewed every 6 months for CDE-connected AI systems. |
| **Req. 8** | Identify users and authenticate | MFA required for all access to CDE-connected AI infrastructure. Service accounts used by LLM agents must be unique, use strong authentication, and be rotated per Req. 8.6. |
| **Req. 10** | Log and monitor all access | AI prompt and output logs in CDE scope must satisfy Req. 10 content requirements. **Log retention: 12 months minimum, 3 months immediately available.** |
| **Req. 11** | Test security of systems and networks | Pen testing (Req. 11.4) must include AI components: gateway APIs, inference endpoints, and prompt injection attack scenarios. Internal scans quarterly; external pen test annually. |
| **Req. 12** | Support information security policies | The AI Acceptable Use Policy and Vendor Intake Questionnaire must be integrated into the entity's overall PCI DSS information security policy structure. TRA required for customized approach implementations. |

#### PCI DSS v4.0 New Requirements with Direct AI Impact

- **Req. 6.4.1 — WAF requirement:** Required for all public-facing web applications. AI gateways and inference APIs exposed to the internet require WAF protection. WAF rulesets should be extended to detect **prompt injection patterns** in addition to standard OWASP web application attack signatures.
- **Req. 6.4.2 — Automated technical solution:** An automated technical solution (WAF, RASP, or equivalent) must be deployed for all public-facing applications. This directly applies to any AI inference API or chatbot interface in CDE scope.
...
| Control Theme | EU AI Act | NIST AI RMF | ISO/IEC 42001 | DISA STIG | CMS ARS | PCI DSS v4.0 |
|--------------|-----------|-------------|---------------|-----------|---------|--------------|
| Risk Management | Art. 9, 10, 15 | Govern / Map | Sect. 6–10 | CAT I/II findings | RA-3 (AIIA) | Req. 12.3 (TRA) |
| Data Governance | Art. 10 | Map / Manage | Support / Ops | SC-28 (at rest) | SC-28, DM-1 | Req. 3, 4 |
| Logging & Audit | Art. 12–13 | Measure / Manage | Perf. Records | AU STIG checks | AU-2, AU-3, AU-9 | Req. 10 |
| Human Oversight | Art. 14 | Govern / Manage | Roles & Comp. | — | AC-6, AT controls | Req. 12 (policy) |
| Security & TEVV | Art. 15 | Measure / Manage | Operations | SCAP/SCC scans | RA-5, CA-8 | Req. 11 (pen test) |
| Config. Mgmt | QMS oblig. | Manage | Policy / Ops | All OS/app STIGs | CM-2, CM-6, CM-7 | Req. 1, 2, 6 |
| Access Control | Art. 9, 14 | Govern | Roles / Ops | AC STIG checks | AC-2, AC-3, AC-6 | Req. 7, 8 |
| Incident Response | Art. 61, 73 | Manage | Corrective Action | IR STIG checks | IR family | Req. 12.10 |
| Supplier Gov. | QMS & suppliers | Govern | Supplier Mgmt | SR-3 (supply chain) | SR-3, SR-5, SR-6 | Req. 12 (TPA) |
| Transparency | Art. 13, 50 | Govern | Operations | — | AP-1, AP-2 | Req. 12 (policy) |
| Cryptography | Art. 15 (cybersec) | Manage | Operations | PKI/Crypto STIG | SC-8, SC-28, IA-5 | Req. 3, 4 |
| Privacy | GDPR (companion) | Map / Manage | Support / Ops | — | PT, AP, DM families | Req. 3 (CHD minim.) |

> **Scoping Note:** DISA STIG and CMS ARS controls apply only to systems in scope for those frameworks (DoD-connected or CMS-connected workloads respectively). PCI DSS applies only to systems in or connected to the Cardholder Data Environment. Scope determination must be documented in the AIIA for each use case.

---

## 16. Quarterly AI Transparency Note
